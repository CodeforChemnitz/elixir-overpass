defmodule Overpass.Parser do
  ...
  """
  @spec parse({:ok, {:xml, String.t}}) :: {:ok, %Overpass.Response{}}
  def parse({:ok, {:xml , response}}) do
    ...
    nodes = osm
            ...
            |> Enum.map(fn (node) ->
              %Overpass.Node{
                ...
                tags: node |> xpath(~x"./tag"l)  |> Enum.map(fn (tag) -> %Overpass.Tag{
                  k: tag |> xpath(~x"./@k"s),
                  v: tag |> xpath(~x"./@v"s)
                }
                end)
    ...
    ways = osm
           ...
           |> Enum.map(fn (way) ->
             %Overpass.Way{
               ...
               tags:  way |> xpath(~x"./tag"l) |> Enum.map(fn (tag) -> %Overpass.Tag{
                 k: tag |> xpath(~x"./@k"s),
                 v: tag |> xpath(~x"./@v"s)
               }
               end)
    ...
    relations = osm
                ...
                |> Enum.map(fn (relation) ->
                  %Overpass.Relation{
                    ...
                    members: relation |> xpath(~x"./member"l) |> Enum.map(fn (member) -> %Overpass.RelationMember{
                      type: member |> xpath(~x"./@type"s),
                      ref:  member |> xpath(~x"./@ref"i),
                      role: member |> xpath(~x"./@role"s)
                    }
                    end),
                    tags: relation |> xpath(~x"./tag"l) |> Enum.map(fn (tag) -> %Overpass.Tag{
                      k: tag |> xpath(~x"./@k"s),
                      v: tag |> xpath(~x"./@v"s)
                    }
                    end)
    ...
    Logger.debug inspect(ways)
    Logger.debug inspect(relations)
  ...
  def parse({:ok, {:json, response}}) do
    ...
    nodes = elems
            ...
            |> Enum.map(fn (node) -> node |> map_node() end)

    ways = elems
           |> Enum.filter(fn %{"type" => type} -> type == "way" end)
           |> Enum.map(fn (way) -> way |> map_way() end)

    relations = elems
                |> Enum.filter(fn %{"type" => type} -> type == "relation" end)
                |> Enum.map(fn (relation) -> relation |> map_relation() end)
  ...
  defp map_relation(%{"id" => id, "members" => members, "tags" => tags}) do
    %Overpass.Relation{
      id:      id,
      tags:    tags    |> Enum.map(fn (tag)    -> tag |> map_tag() end),
      members: members |> Enum.map(fn (member) ->
